...
class DownloaderWindow(QtWidgets.QWidget):
    ...
    def check_ffmpeg(self):
        if shutil.which('ffmpeg') is None:
            ...
            try:
                subprocess.run([sys.executable, '-m', 'spotdl', '--download-ffmpeg'], check=True, capture_output=True, timeout=30)
    ...
    def _download_thread(self):
        ...
        try:
            if platform == 'Spotify':
                # use spotdl
                cmd = [sys.executable, '-m', 'spotdl', 'download', url, '--output', self.dir_edit.text() or os.path.join(os.path.dirname(__file__), 'Downloads')]
                subprocess.run(cmd, check=True)
            else:
                ydl_opts = {'format': 'bestaudio/best' if format_type == 'MP3' else 'best', 'outtmpl': '%(title)s.%(ext)s'}
